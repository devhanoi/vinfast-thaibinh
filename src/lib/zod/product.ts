...
export const ProductEntity = z.object({
  ...
  segment: z.string().nullish().transform((v) => v ?? null),
  ...
  battery: z.string().nullish().transform((v) => v ?? null),
  rangeKm: z.number().int().nullish().transform((v) => v ?? null),
  rangeText: z.string().nullish().transform((v) => v ?? null),
